...
public:
  ImageFrame(){};
  ImageFrame(
      const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &_points,
    ...
    Eigen::Quaterniond Q = Eigen::Quaterniond(
        _lidar_initialization_info[7], _lidar_initialization_info[4],
...
#if IF_OFFICIAL
    q_lidar_to_cam = tf::Quaternion(
        0, 1, 0, 0); // rotate orientation // mark: camera - lidar
    q_lidar_to_cam_eigen = Eigen::Quaterniond(
        0, 0, 0,
        1); // rotate position by pi, (w, x, y, z) // mark: camera - lidar
#else
    //? mod：添加自己的旋转外参变化，即R_lidar_imu
    Eigen::Quaterniond q_lidar_imu(R_lidar_imu_);
    q_lidar_imu.normalize(); // 四元数归一化
    q_lidar_to_cam = tf::Quaternion(q_lidar_imu.x(), q_lidar_imu.y(),
                                    q_lidar_imu.z(), q_lidar_imu.w());
#endif
  }

  // convert odometry from ROS Lidar frame to VINS camera frame
  vector<float> getOdometry(deque<nav_msgs::Odometry> &odomQueue,
                            double img_time) {
    vector<float> odometry_channel;
    odometry_channel.resize(
        18, -1); // reset id(1), P(3), Q(4), V(3), Ba(3), Bg(3), gravity(1)
    ...
    // convert odometry rotation from lidar ROS frame to VINS camera frame (only
    // rotation, assume lidar, camera, and IMU are close enough)
    tf::Quaternion q_odom_lidar;
    tf::quaternionMsgToTF(odomCur.pose.pose.orientation, q_odom_lidar);

#if IF_OFFICIAL
    tf::Quaternion q_odom_cam =
        tf::createQuaternionFromRPY(0, 0, M_PI) *
        (q_odom_lidar *
         q_lidar_to_cam); // global rotate by pi // mark: camera - lidar
#else
    //? mod:
    //vins_world坐标系和odom坐标系不再绕着Z轴旋转，而是直接对齐，也就是前面不乘
    //tf::createQuaternionFromRPY(0, 0, M_PI) 了 ; R_odom_imu = R_odom_lidar *
    //R_lidar_imu
    tf::Quaternion q_odom_cam =
        q_odom_lidar *
        q_lidar_to_cam; // global rotate by pi // mark: camera - lidar
    ...
    //;            = [R_odom_lidar * R_lidar_imu, R_odom_lidar * t_lidar_imu +
    //t_odom_lidar]
    Eigen::Quaterniond q_wl(q_odom_lidar.w(), q_odom_lidar.x(),
