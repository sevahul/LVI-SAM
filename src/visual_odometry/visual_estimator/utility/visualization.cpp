...
void registerPub(ros::NodeHandle &n) {
  pub_latest_odometry = n.advertise<nav_msgs::Odometry>(
      PROJECT_NAME + "/vins/odometry/imu_propagate", 1000);
  //! 重要：发布T_odom_lidar给LIO系统，用于LIO系统后端scan-to-map的位姿初值估计
  pub_latest_odometry_ros = n.advertise<nav_msgs::Odometry>(
      PROJECT_NAME + "/vins/odometry/imu_propagate_ros", 1000);
  pub_path =
      n.advertise<nav_msgs::Path>(PROJECT_NAME + "/vins/odometry/path", 1000);
  pub_odometry = n.advertise<nav_msgs::Odometry>(
      PROJECT_NAME + "/vins/odometry/odometry", 1000);
  pub_point_cloud = n.advertise<sensor_msgs::PointCloud>(
      PROJECT_NAME + "/vins/odometry/point_cloud", 1000);
  pub_margin_cloud = n.advertise<sensor_msgs::PointCloud>(
      PROJECT_NAME + "/vins/odometry/history_cloud", 1000);
  pub_key_poses = n.advertise<visualization_msgs::Marker>(
      PROJECT_NAME + "/vins/odometry/key_poses", 1000);
  pub_camera_pose = n.advertise<nav_msgs::Odometry>(
      PROJECT_NAME + "/vins/odometry/camera_pose", 1000);
  pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>(
      PROJECT_NAME + "/vins/odometry/camera_pose_visual", 1000);
  pub_keyframe_pose = n.advertise<nav_msgs::Odometry>(
      PROJECT_NAME + "/vins/odometry/keyframe_pose", 1000);
  pub_keyframe_point = n.advertise<sensor_msgs::PointCloud>(
      PROJECT_NAME + "/vins/odometry/keyframe_point", 1000);
  pub_extrinsic = n.advertise<nav_msgs::Odometry>(
      PROJECT_NAME + "/vins/odometry/extrinsic", 1000);
...
tf::Transform transformConversion(const tf::StampedTransform &t) {
  ...
  return tf::Transform(tf::createQuaternionFromRPY(rollCur, pitchCur, yawCur),
                       tf::Vector3(xCur, yCur, zCur));
  ;
}
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q,
                       ...
                       const int &failureId, const Eigen::Vector3d &t_ic,
...
#if IF_OFFICIAL
  // imu odometry in ROS format (change rotation), used for lidar odometry
  // initial guess
  odometry.pose.covariance[0] =
      double(failureId); // notify lidar odometry failure
  ...
  // TF of camera in vins_world in ROS format (change rotation), used for depth
  // registration
  tf::Transform t_w_body =
      tf::Transform(q_odom_ros, tf::Vector3(P.x(), P.y(), P.z()));
  ...
  //; R_imu_lidar
  tf::Quaternion q_imu_lidar_tf(q_imu_lidar.x(), q_imu_lidar.y(),
                                q_imu_lidar.z(), q_imu_lidar.w());
  ...
  tf::Transform t_w_body =
      tf::Transform(q_odom_imu, tf::Vector3(P.x(), P.y(), P.z()));
  ...
  // Step 3: 发布camera 和
  // IMU之间的外参，这个在动态估计外参的时候会变换，所以前端深度注册也要使用这个动态外参
  //;
  //另外这里发布外参的时候直接发布了vins相机的FLU坐标系的变换，因为前端深度注册要用相机的FLU坐标系
  tf::Transform transform;
  tf::Quaternion q;
  //; Eigen::Quaterniond(0.5, 0.5, -0.5, 0.5)对应的旋转矩阵是R_c_cFLU，即相机的
  //; 前左上坐标系 -> 正常的相机右下前坐标系 之间的旋转，结果就是[0, -1, 0; 0,
  // 0, -1; 1, 0, 0] ; R_imu_cFLU = R_imu_cam * R_cam_camFLU
  ...
  br.sendTransform(tf::StampedTransform(transform, header.stamp,
                                        "vins_body_imuhz", "vins_cameraFLU"));
#endif

  if (ALIGN_CAMERA_LIDAR_COORDINATE) {
#if IF_OFFICIAL
    static tf::Transform t_odom_world = tf::Transform(
        tf::createQuaternionFromRPY(0, 0, M_PI), tf::Vector3(0, 0, 0));
#else
    ...
    if (header.stamp.toSec() - last_align_time > 1.0) {
      try {
        ...
        //; 即T_odom_lidar =
        // T_vinsworld_lidar，其中T_odom_lidar是LIO估计的结果，T_vinsworld_lidar
        //; 是VINS估计的结果，则T_odom_vinsworld = T_odom_lidar *
        // T_lidar_vinsworld
        tf::StampedTransform T_odom_lidar;
        listener.lookupTransform("odom", "base_link", ros::Time(0),
                                 T_odom_lidar);
        tf::Transform t_w_lidar = tf::Transform(
            q_odom_lidar,
            tf::Vector3(t_odom_lidar.x(), t_odom_lidar.y(), t_odom_lidar.z()));
        tf::StampedTransform T_vinsworld_lidar =
            tf::StampedTransform(t_w_lidar, header.stamp, "vinsworld", "lidar");
    ...
    br.sendTransform(
        tf::StampedTransform(t_odom_world, header.stamp, "odom", "vins_world"));
  } else {

#if IF_OFFICIAL
    static tf::Transform t_static = tf::Transform(
        tf::createQuaternionFromRPY(0, 0, M_PI), tf::Vector3(0, 0, 0));
#else
    //? mod: vins_world坐标系和odom坐标系不再绕着Z轴旋转，而是直接对齐
    static tf::Transform t_static = tf::Transform(
    ...
    br.sendTransform(
        tf::StampedTransform(t_static, header.stamp, "odom", "vins_world"));
  }
}
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  transform.setRotation(q);
  br.sendTransform(
      tf::StampedTransform(transform, header.stamp, "vins_world", "vins_body"));
  ...
  transform.setRotation(q);
  br.sendTransform(tf::StampedTransform(transform, header.stamp, "vins_body",
                                        "vins_camera"));
...
void pubKeyframe(const Estimator &estimator) {
  if (pub_keyframe_pose.getNumSubscribers() == 0 &&
      pub_keyframe_point.getNumSubscribers() == 0)
    return;
