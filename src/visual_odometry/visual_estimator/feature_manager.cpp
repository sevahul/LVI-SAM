...
bool FeatureManager::addFeatureCheckParallax(
  ...
  for (auto &id_pts : image) {
    ...
    if (it == feature.end()) {
      // this feature in the image is observed for the first time, create a new
      // feature object
      ...
      // (initialize initial feature depth with current image depth is not
      // exactly accurate if camera moves very fast, then lines bebow can be
      // commented out)
...
VectorXd FeatureManager::getDepthVector() {
  ...
  for (auto &it_per_id : feature) {
    ...
    // optimized depth after ceres maybe negative, initialize them with default
    // value for this optimization
    if (it_per_id.estimated_depth > 0)
    ...
    if (it->start_frame != 0)
      ...
      if (it->feature_per_frame.size() < 2) {
        ...
        Eigen::Vector3d pts_i =
            uv_i *
            depth; // feature in cartisian space in old local camera frame
        Eigen::Vector3d w_pts_i =
            marg_R * pts_i +
            marg_P; // feautre in cartisian space in world frame
        Eigen::Vector3d pts_j =
            new_R.transpose() *
            (w_pts_i -
             new_P); // feature in cartisian space in shifted local camera frame
        double dep_j = pts_j(2);

        // after deletion, the feature has lidar depth in the first of the
        // remaining frame
        if (it->feature_per_frame[0].depth > 0) {
