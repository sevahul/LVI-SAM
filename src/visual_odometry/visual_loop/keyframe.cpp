...
KeyFrame::KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i,
  ...
  cv::resize(image, thumbnail, cv::Size(), MATCH_IMAGE_SCALE,
             MATCH_IMAGE_SCALE);
  ...
  if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
    ...
    if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
      if (pub_match_img.getNumSubscribers() != 0) {
        int gap = 10;
        cv::Mat gap_image(thumbnail.size().height, gap, CV_8UC1,
        ...
        for (int i = 0; i < (int)matched_2d_cur.size(); i++) {
          ...
          cv::circle(loop_match_img, cur_pt, 5 * MATCH_IMAGE_SCALE,
                     cv::Scalar(0, 255, 0));
        }
        ...
        for (int i = 0; i < (int)matched_2d_old.size(); i++) {
          ...
          cv::circle(loop_match_img, old_pt, 5 * MATCH_IMAGE_SCALE,
                     cv::Scalar(0, 255, 0));
        }
        ...
        for (int i = 0; i < (int)matched_2d_cur.size(); i++) {
          cv::Point2f old_pt = matched_2d_old[i] * MATCH_IMAGE_SCALE;
          old_pt.x += thumbnail.size().width + gap;
          cv::line(loop_match_img, matched_2d_cur[i] * MATCH_IMAGE_SCALE,
                   old_pt, cv::Scalar(0, 255, 0), 2 * MATCH_IMAGE_SCALE, 8, 0);
        }
        // plot text
        int banner_height = (double)100 * MATCH_IMAGE_SCALE;
        cv::Mat notation(banner_height,
                         thumbnail.size().width + gap + thumbnail.size().width,
        ...
        putText(
            notation, "previous frame: " + to_string(old_kf->index),
            cv::Point2f(5 + thumbnail.size().width + gap, banner_height - 5),
            cv::FONT_HERSHEY_SIMPLEX, MATCH_IMAGE_SCALE * 2, cv::Scalar(255),
            2);
        cv::vconcat(notation, loop_match_img, loop_match_img);
        // publish matched image
        sensor_msgs::ImagePtr msg =
            cv_bridge::CvImage(std_msgs::Header(), "bgr8", loop_match_img)
                .toImageMsg();
