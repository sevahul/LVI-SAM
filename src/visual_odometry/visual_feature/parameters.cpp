...
void readParameters(ros::NodeHandle &n) {
  ...
  Transform_imu_lidar = tf::Transform(
      tf::Quaternion(Q_imu_lidar.x(), Q_imu_lidar.y(), Q_imu_lidar.z(),
                     Q_imu_lidar.w()),
      tf::Vector3(t_imu_lidar(0), t_imu_lidar(1), t_imu_lidar(2)));

  ROS_WARN_STREAM(
      "=vins-feature_tracker read R_imu_lidar : =====================");
  std::cout << R_imu_lidar << std::endl;
  ROS_WARN_STREAM(
      "=vins-feature_tracker read t_lidar_imu : =====================");
  std::cout << t_imu_lidar(0) << ", " << t_imu_lidar(1) << ", "
            << t_imu_lidar(2) << std::endl;
#endif
