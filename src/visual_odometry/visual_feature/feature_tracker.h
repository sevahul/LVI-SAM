...
public:
  ...
  get_depth(const ros::Time &stamp_cur, const cv::Mat &imageCur,
...
#else
      //? mod:
      //直接监听vins_camFLU坐标系在世界坐标系下的表示，这样就把VIO的动态外参包括进去了
      listener.waitForTransform("vins_world", "vins_cameraFLU", stamp_cur,
                                ros::Duration(0.01));
      listener.lookupTransform("vins_world", "vins_cameraFLU", stamp_cur,
                               transform);
#endif
    ...
    *depth_cloud_local = *depth_cloud_local_filter2;
    ...
    // visualize features in cartesian 3d space (including the feature without
    // depth (default 1))
#if IF_OFFICIAL
    publishCloud(&pub_depth_feature, features_3d_sphere, stamp_cur,
                 "vins_body_ros");
#else
    //? mod：发布点云坐标系
    publishCloud(&pub_depth_feature, features_3d_sphere, stamp_cur,
    ...
    if (pub_depth_image.getNumSubscribers() != 0) {
      ...
      cv::addWeighted(showImage, 1.0, circleImage, 0.7, 0,
                      showImage); // blend camera image and circle image
