...
public:
  ...
  mapOptimization() {
    ...
    pubKeyPoses = nh.advertise<sensor_msgs::PointCloud2>(
        PROJECT_NAME + "/lidar/mapping/trajectory", 1);
    pubLaserCloudSurround = nh.advertise<sensor_msgs::PointCloud2>(
        PROJECT_NAME + "/lidar/mapping/map_global", 1);
    pubOdomAftMappedROS = nh.advertise<nav_msgs::Odometry>(
        PROJECT_NAME + "/lidar/mapping/odometry", 1);
    pubPath =
        nh.advertise<nav_msgs::Path>(PROJECT_NAME + "/lidar/mapping/path", 1);
    ...
    pubRecentKeyFrames = nh.advertise<sensor_msgs::PointCloud2>(
        PROJECT_NAME + "/lidar/mapping/map_local", 1);
  ...
  void visualizeGlobalMapThread() {
    ...
    for (int i = 0; i < (int)cloudKeyPoses3D->size(); i++) {
      // clip cloud
      // pcl::PointCloud<PointType>::Ptr cornerTemp(new
      // pcl::PointCloud<PointType>()); pcl::PointCloud<PointType>::Ptr
      // cornerTemp2(new pcl::PointCloud<PointType>()); *cornerTemp =
      // *transformPointCloud(cornerCloudKeyFrames[i],
      // &cloudKeyPoses6D->points[i]); for (int j = 0; j <
      // (int)cornerTemp->size(); ++j)
      // {
      ...
      // }
      // pcl::PointCloud<PointType>::Ptr surfTemp(new
      // pcl::PointCloud<PointType>()); pcl::PointCloud<PointType>::Ptr
      // surfTemp2(new pcl::PointCloud<PointType>()); *surfTemp =
      // *transformPointCloud(surfCloudKeyFrames[i],
      // &cloudKeyPoses6D->points[i]); for (int j = 0; j <
      // (int)surfTemp->size(); ++j)
      // {
      //     if (surfTemp->points[j].z > cloudKeyPoses6D->points[i].z &&
      //     surfTemp->points[j].z < cloudKeyPoses6D->points[i].z + 5)
  ...
  void performLoopClosure(const std_msgs::Float64MultiArray &loopMsg) {
    ...
    {
      // if image loop closure comes at high frequency, many image loop may
      // point to the same key_cur
    ...
    pcl::PointCloud<PointType>::Ptr prevKeyframeCloud(
        new pcl::PointCloud<PointType>());
    {
      loopFindNearKeyframes(copy_cloudKeyPoses6D, cureKeyframeCloud, key_cur,
                            0);
    ...
    Eigen::Affine3f pose_diff_t; // serves as initial guess
    {
      ...
      if (t_diff.norm() < historyKeyframeSearchRadius)
        t_diff.setZero();
      pose_diff_t =
          pcl::getTransformation(t_diff.x(), t_diff.y(), t_diff.z(), 0, 0, 0);
    }
    ...
    // initial guess cloud
    pcl::PointCloud<PointType>::Ptr cureKeyframeCloud_new(
        new pcl::PointCloud<PointType>());
    pcl::transformPointCloud(*cureKeyframeCloud, *cureKeyframeCloud_new,
                             pose_diff_t);
    ...
    if (icp.getFitnessScore() < historyKeyframeFitnessScore &&
      ...
      gtsam::Pose3 poseFrom =
          affine3fTogtsamPose3(Eigen::Affine3f(icp.getFinalTransformation()) *
                               pose_diff_t * pose_cur);
      gtsam::Pose3 poseTo =
          pclPointTogtsamPose3(copy_cloudKeyPoses6D->points[key_pre]);
      // get noise
  ...
  void loopFindNearKeyframes(
    ...
    for (int i = -searchNum; i <= searchNum; ++i) {
      ...
      if (key_near < 0 || key_near >= cloudSize)
        continue;
      *nearKeyframes +=
          *transformPointCloud(cornerCloudKeyFrames[key_near],
                               &copy_cloudKeyPoses6D->points[key_near]);
      *nearKeyframes +=
          *transformPointCloud(surfCloudKeyFrames[key_near],
                               &copy_cloudKeyPoses6D->points[key_near]);
    }
  ...
  void
  loopFindKey(const std_msgs::Float64MultiArray &loopMsg,
              const pcl::PointCloud<PointTypePose>::Ptr &copy_cloudKeyPoses6D,
  ...
  void performLoopClosureDetection() {
    ...
    // find latest key and time
    {
      ...
      kdtreeHistoryKeyPoses->radiusSearch(
          cloudKeyPoses3D->back(), historyKeyframeSearchRadius,
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  bool LMOptimization(int iterCount) {
    // This optimization is from the original loam_velodyne, need to cope with
    // coordinate transformation lidar <- camera      ---     camera <- lidar x
    // = z                ---     x = y y = x                ---     y = z z = y
    // ---     z = x roll = yaw           ---     roll = pitch pitch = roll ---
    // pitch = yaw yaw = pitch          ---     yaw = roll
  ...
  void addOdomFactor() {
    if (cloudKeyPoses3D->points.empty()) {
      ...
      // if (isDegenerate)
      // {
      // adding VINS constraints is deleted as benefits are not obvious, disable
      // for now gtSAMgraph.add(BetweenFactor<Pose3>(cloudKeyPoses3D->size()-1,
      // cloudKeyPoses3D->size(), vinsPoseFrom.between(vinsPoseTo),
      // odometryNoise));
      // }
    }
  }
  ...
  void addGPSFactor() {
    ...
    if (cloudKeyPoses3D->points.empty())
      return;
    else if (pointDistance(cloudKeyPoses3D->front(), cloudKeyPoses3D->back()) <
             5.0)
      return;
