...
public:
  ...
  //; 定义旋转到和LiDAR坐标轴完全平行、但是坐标系原点不变的IMU坐标系为imulidar,
  //那么这里要的其实是T_imulidar_lidar ;
  //而配置文件中我们给的extTrans是T_imu_lidar的平移部分，即t_imu_lidar，所以这里还需要转换一步。
  //; T_imu_imulidar = [R_imu_lidar, 0; 0 1], T_imu_lidar = [R_imu_lidar,
  // t_imu_lidar; 0, 1] ; T_imulidar_lidar = [I, R_lidar_imu*t_imu_lidar; 0, 1]
  Eigen::Vector3d t_imulidar_lidar = R_lidar_imu * t_imu_lidar;
  gtsam::Pose3 imu2Lidar =
      gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0),
                   gtsam::Point3(t_imulidar_lidar.x(), t_imulidar_lidar.y(),
                                 t_imulidar_lidar.z()));
  //; T_lidar_imulidar, 同上
  gtsam::Pose3 lidar2Imu =
      gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0),
  ...
  IMUPreintegration() {
    ...
    subOdometry = nh.subscribe<nav_msgs::Odometry>(
        PROJECT_NAME + "/lidar/mapping/odometry", 5,
        &IMUPreintegration::odometryHandler, this,
        ros::TransportHints().tcpNoDelay());

    pubImuOdometry = nh.advertise<nav_msgs::Odometry>("odometry/imu", 2000);
    pubImuPath =
        nh.advertise<nav_msgs::Path>(PROJECT_NAME + "/lidar/imu/path", 1);

    map_to_odom = tf::Transform(tf::createQuaternionFromRPY(0, 0, 0),
                                tf::Vector3(0, 0, 0));
  ...
  void imuHandler(const sensor_msgs::Imu::ConstPtr &imuMsg) {
    ...
    if (imuTime - last_path_time > 0.1) {
      ...
      while (!imuPath.poses.empty() &&
             abs(imuPath.poses.front().header.stamp.toSec() -
                 imuPath.poses.back().header.stamp.toSec()) > 3.0)
