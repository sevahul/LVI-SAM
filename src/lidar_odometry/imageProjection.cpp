...
public:
  ImageProjection() : deskewFlag(0) {
    ...
    pubLaserCloudInfo = nh.advertise<lvi_sam::cloud_info>(
        PROJECT_NAME + "/lidar/deskew/cloud_info", 5);
  ...
  bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg) {
    ...
    if (ringFlag == 0) {
      ...
      if (ringFlag == -1) {
        ROS_ERROR("Point cloud ring channel not available, please configure "
    ...
    if (deskewFlag == 0) {
      ...
      if (deskewFlag == -1)
        ROS_WARN("Point cloud timestamp not available, deskew function "
  ...
  void projectPointCloud() {
    ...
    for (int i = 0; i < cloudSize; ++i) {
      ...
      // if (range < 6.0 && rowIdn <= 7 && (columnIdn >= 1600 || columnIdn <=
      // 200))
      //     continue;
      // if (thisPoint.z < -2.0)
      //     continue;
