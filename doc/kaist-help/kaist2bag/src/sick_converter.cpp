...
SickConverter::SickConverter(const std::string &dataset_dir,
                             const std::string &save_dir,
                             const std::string &back_topic,
                             const std::string &middle_topic)
    : Converter(dataset_dir, save_dir), back_topic_(back_topic),
      middle_topic_(middle_topic) {
...
int SickConverter::Convert() {
  ...
  boost::filesystem::path back_bag_file =
      boost::filesystem::path(save_dir_) / back_bag_name_;
  boost::filesystem::path middle_bag_file =
      boost::filesystem::path(save_dir_) / middle_bag_name_;

  const std::string back_stamp_file =
      dataset_dir_ + "/" + default_back_stamp_file;
  const std::string back_data_dir = dataset_dir_ + "/" + default_back_data_dir;
  const std::string middle_stamp_file =
      dataset_dir_ + "/" + default_middle_stamp_file;
  const std::string middle_data_dir =
      dataset_dir_ + "/" + default_middle_data_dir;

  ROS_INFO("saving %s", back_bag_file.c_str());
  Convert(back_stamp_file, back_data_dir, back_bag_file.string(), back_topic_,
          "back_sick");
  ROS_INFO("done saving %s", back_bag_file.c_str());
  ROS_INFO("saving %s", middle_bag_file.c_str());
  Convert(middle_stamp_file, middle_data_dir, middle_bag_file.string(),
          middle_topic_, "middle_sick");
  ROS_INFO("done saving %s", middle_bag_file.c_str());

  return 0;
}

void SickConverter::Convert(const std::string &stamp_file,
