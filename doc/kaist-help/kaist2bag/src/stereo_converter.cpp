...
StereoConverter::StereoConverter(const std::string &dataset_dir,
...
int StereoConverter::Convert() {
  ...
  // boost::filesystem::path right_bag_file = boost::filesystem::path(save_dir_)
  // / right_bag_name_;
  ...
  // const std::string right_data_dir = dataset_dir_ + "/" +
  ...
  // "/stereo/right"); ROS_INFO("done saving %s", right_bag_file.c_str());
