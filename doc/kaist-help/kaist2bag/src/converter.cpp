...
Converter::Converter(const std::string &dataset_dir,
...
int Converter::Convert() { return 0; }

bool Converter::DirValid() { return dir_valid_; }

void Converter::CheckAndCreateSaveDir() {
  if (!boost::filesystem::is_directory(save_dir_) ||
      !boost::filesystem::exists(save_dir_)) {
