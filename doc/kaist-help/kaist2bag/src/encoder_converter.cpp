...
EncoderConverter::EncoderConverter(const std::string &dataset_dir,
...
int EncoderConverter::Convert() {
  ...
  while (fscanf(fp, "%ld,%ld,%ld\n", &stamp, &left_count, &right_count) == 3) {
    ...
    if (has_previous) {
      ...
      double degl =
          ((double)d_left_cnt / (double)encoder_resolution_) * 2 * M_PI;
      double degr =
          ((double)d_right_cnt / (double)encoder_resolution_) * 2 * M_PI;
