...
int main(int argc, char **argv) {
  ...
  std::vector<std::string> sensor_names = {"altimeter", "encoder", "fog",
                                           "gps",       "vrs",     "imu",
                                           "velodyne",  "sick",    "stereo"};
  ...
  if (sensors[SensorType::kEncoder]) {
    ...
    EncoderConverter encoder(dataset, save_to, encoder_irp_topic,
                             encoder_raw_topic);
    encoder.Convert();
  }
  ...
  if (sensors[SensorType::kStereo]) {
    ...
    StereoConverter stereo(dataset, save_to, stereo_left_topic,
                           stereo_right_topic);
    stereo.Convert();
  }
