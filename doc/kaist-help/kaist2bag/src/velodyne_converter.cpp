...
VelodyneConverter::VelodyneConverter(const std::string &dataset_dir,
                                     const std::string &save_dir,
                                     const std::string &left_topic,
                                     const std::string &right_topic)
    : Converter(dataset_dir, save_dir), left_topic_(left_topic),
...
int VelodyneConverter::Convert() {
  ...
  // const std::string right_stamp_file = dataset_dir_ + "/" +
  // default_right_stamp_file; const std::string right_data_dir = dataset_dir_ +
  // "/" + default_right_data_dir;
  ...
  // right_topic_, "right_velodyne"); ROS_INFO("done saving %s",
  // right_bag_file.c_str());

  return 0;
}

void VelodyneConverter::RecoverVLP16Timestamp(
    const VPointCloud::Ptr input_cloud, RTPointCloud::Ptr output_cloud) {
